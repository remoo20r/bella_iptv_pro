...
// --- محرك التشغيل (VLC أو المشغل المدمج) ---
enum PlayerEngine { vlc, exo }

class PlayerPrefs {
  static const _liveKey = 'player_engine_live';
  static const _vodKey = 'player_engine_vod';

  // الافتراضي هو VLC لكل الأقسام
  static Future<PlayerEngine> getLiveEngine() async {
    final prefs = await SharedPreferences.getInstance();
    return (prefs.getString(_liveKey) == 'exo') ? PlayerEngine.exo : PlayerEngine.vlc;
  }

  static Future<void> setLiveEngine(PlayerEngine engine) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_liveKey, engine == PlayerEngine.exo ? 'exo' : 'vlc');
  }

  static Future<PlayerEngine> getVodEngine() async {
    final prefs = await SharedPreferences.getInstance();
    return (prefs.getString(_vodKey) == 'exo') ? PlayerEngine.exo : PlayerEngine.vlc;
  }

  static Future<void> setVodEngine(PlayerEngine engine) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_vodKey, engine == PlayerEngine.exo ? 'exo' : 'vlc');
  }
}

/// مشغل موحّد يدعم VLC أو المشغل المدمج (ExoPlayer على أندرويد) بنفس الواجهة.
/// يحل مشكلة ظهور الفيديو صغيراً أعلى الشاشة عبر إعطاء المشغل قياساً صريحاً
/// دائماً (LayoutBuilder) وإعادة بناء الـ platform view كلما تغير القياس
/// أو المصدر (عبر key)، بدل الاعتماد على "resize" ضمني قد لا يحدث.
class UnifiedPlayer extends StatefulWidget {
  ...
  const UnifiedPlayer({
    ...
    this.autoPlay = true,
    this.showBuiltInControls = false,
  });

  @override
  State<UnifiedPlayer> createState() => UnifiedPlayerState();
}

class UnifiedPlayerState extends State<UnifiedPlayer> {
  VlcPlayerController? _vlcController;
  VideoPlayerController? _exoController;
  bool _hasError = false;

  bool get isPlaying {
    if (widget.engine == PlayerEngine.vlc) {
  ...
  @override
  void dispose() {
    _vlcController?.removeListener(_onUpdate);
    _vlcController?.stopRendererScanning();
    _vlcController?.dispose();
    _exoController?.removeListener(_onUpdate);
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (widget.engine == PlayerEngine.vlc) {
          ...
          // مفتاح فريد مرتبط بالمصدر والقياس الفعلي: يجبر فلاتر على إعادة
          // إنشاء الـ platform view كاملاً بدل محاولة تحجيمه، وهذا ما يمنع
          // بقاء الفيديو "عالقاً" بحجمه القديم (مشكلة الشريط الصغير أعلى الشاشة).
          return SizedBox(
            width: w,
            height: h,
            child: VlcPlayer(
              key: ValueKey('vlc-${widget.url}-${w.toStringAsFixed(0)}x${h.toStringAsFixed(0)}'),
              controller: c,
              aspectRatio: w / h,
              placeholder: const Center(child: CircularProgressIndicator(color: AppColors.gold)),
            ),
          );
        }
...
class _LiveChannelsScreenState extends State<LiveChannelsScreen> with AutoHideControls<LiveChannelsScreen> {
  ...
  PlayerEngine _liveEngine = PlayerEngine.vlc;
  ...
  Future<void> _loadEngine() async {
    final engine = await PlayerPrefs.getLiveEngine();
    if (mounted) setState(() => _liveEngine = engine);
  }
  ...
  String get _currentStreamUrl =>
      "$SERVER_URL/live/${widget.username}/${widget.password}/$_selectedStreamId.ts";
  ...
  Widget _buildVideoArea() {
    if (_selectedStreamId.isEmpty) {
      return Container(color: Colors.black);
    }

    return Container(
      color: Colors.black,
      // مفتاح مرتبط بالقناة وحالة ملء الشاشة: يجبر إعادة إنشاء المشغل
      // كاملاً بدل تحجيمه، وهذا يمنع بقاء الفيديو صغيراً أعلى الشاشة.
      child: UnifiedPlayer(
        key: ValueKey('live-$_selectedStreamId-$_isFullscreen'),
...
class _SimpleVideoPlayerScreenState extends State<SimpleVideoPlayerScreen>
  ...
  final GlobalKey<UnifiedPlayerState> _playerKey = GlobalKey<UnifiedPlayerState>();
  ...
  Future<void> _loadEngine() async {
    final engine = await PlayerPrefs.getVodEngine();
    if (mounted) setState(() => _engine = engine);
  }

  void _seek(int seconds) {
    _playerKey.currentState?.seekRelative(seconds);
  }
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          _controlButton(
                            (_playerKey.currentState?.isPlaying ?? false) ? Icons.pause : Icons.play_arrow,
                            () {
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  PlayerEngine _liveEngine = PlayerEngine.vlc;
  PlayerEngine _vodEngine = PlayerEngine.vlc;
  ...
  Future<void> _loadEngines() async {
    final live = await PlayerPrefs.getLiveEngine();
    final vod = await PlayerPrefs.getVodEngine();
    if (mounted) setState(() {
      _liveEngine = live;
      _vodEngine = vod;
    });
  ...
  Widget _playerEngineSelector({
    required String label,
    required IconData icon,
    required PlayerEngine value,
    required ValueChanged<PlayerEngine> onChanged,
  }) {
    Widget optionButton(String text, PlayerEngine engine) {
      final selected = value == engine;
      return Expanded(
        child: RemoteFocusable(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: selected ? AppColors.red : Colors.black.withOpacity(0.35),
              borderRadius: BorderRadius.circular(10),
              border: Border.all(
                color: selected ? AppColors.gold : AppColors.gold.withOpacity(0.25),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              optionButton("VLC", PlayerEngine.vlc),
              const SizedBox(width: 10),
              optionButton("المشغل المدمج", PlayerEngine.exo),
            ],
          ),
        ],
      ),
    ...
    return GradientScaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 28),
          const Text("مشغل الفيديو", style: TextStyle(color: AppColors.gold, fontSize: 15, fontWeight: FontWeight.bold)),
          const SizedBox(height: 10),
          _playerEngineSelector(
            label: "البث المباشر (Live TV)",
            icon: Icons.live_tv,
            value: _liveEngine,
            onChanged: (engine) {
              setState(() => _liveEngine = engine);
              PlayerPrefs.setLiveEngine(engine);
            },
          ),
          const SizedBox(height: 12),
          _playerEngineSelector(
            ...
            value: _vodEngine,
            onChanged: (engine) {
              setState(() => _vodEngine = engine);
              PlayerPrefs.setVodEngine(engine);
            },
          ),
          const SizedBox(height: 28),
...
class _SimpleVideoPlayerScreenState extends State<SimpleVideoPlayerScreen>
  ...
  void _seek(int seconds) async {
    final controller = _controller;
    if (controller == null) return;
    final current = await controller.getPosition();
    if (current != null) {
      final target = current + Duration(seconds: seconds);
